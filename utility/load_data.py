...
class DataHandler(object):
    ...
    def LoadDataDenoised(self):
        ...
        self.trnMats = [sp.dok_matrix((self.n_users, self.n_items), dtype=np.float32) for i in
                        range(len(self.behs))]
        self.augMat = sp.dok_matrix((self.n_users, self.n_items), dtype=np.float32)
        ...
        # 对每个行为中，读取行为文件
        # 记录当前行为的交互次数
        with open(f"denoised/denoised_{self.dataset_name}-low-0.3.pkl", 'rb') as file:
            denoised_data = pickle.load(file)
        for i in range(len(self.behs) - 1):
            beh = self.behs[i]
            self.trnMats[i] = denoised_data[i].todok()
            for uid in range(denoised_data[i].shape[0]):
                items_list = denoised_data[i][uid].indices.tolist()
        ...
        # 单独处理train文件
        # 记录交互次数        
        self.trnMats[-1] = denoised_data[-1].todok()
        for uid in range(denoised_data[-1].shape[0]):
            items_list = denoised_data[-1][uid].indices.tolist()
            self.interNum[-1] += len(items_list)
            
            # 填入交互字典列表
            self.trnDicts[-1][uid] = items_list
        ...
        # 处理aug文件
        aug_file = "denoised/" + self.dataset_name + "-aug0.4.npz"
    ...
    def LoadData(self):
        ...
        # 处理aug文件
        aug_file = self.predir + "/aug.txt"
        with open(aug_file) as f:
            for l in f.readlines():
                if len(l) == 0: break
                l = l.strip('\n')
                items = [int(i) for i in l.split(' ')]
                uid, aug_items = items[0], items[1:]
                self.augNum += len(aug_items)
                # 填入稀疏矩阵，填入字典列表
                for i in aug_items:
                    self.augMat[uid, i] = 1.        
